...
export class MdFilter extends BaseMessageFilter {
  ...
  async __md2jpg(sender: Sender, content: string) {
    ...
    try {
      const b64 = await md2jpg((await genTemplate(content)))
...
async function genTemplate(md: string) {
  let short = ""
