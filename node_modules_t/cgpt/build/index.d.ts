...
declare abstract class ConversationSignal {
    cancel(val: string): void;
}
type QueueReply = string | ((reply: (s: string) => Promise<ChatResponse>) => Promise<string>);
...
declare class FunctionExecutor extends Queue<(error?: ChatGPTError) => void> {
    protected _timer: NodeJS.Timer | null;
    protected _pingTimer: NodeJS.Timer | null;
    protected _pingMs: number;
    protected _pingExecutor: () => void;
    protected _pingCount: number;
    protected _previousSleepMs: number;
    constructor(opt: {
        maximum: number;
        pingMs?: number;
        pingExecutor?: () => void;
    });
    setPingExecutor(executor: () => void): void;
    yauld(): void;
    sleep(timeoutMs?: number): void;
    close(): void;
}
...
declare class ChatGPTAPIBrowser extends AChatGPTAPI {
    ...
    protected _executor: FunctionExecutor;
    protected _userIdCache: Map<string, {
        conversationId?: string;
        parentMessageId?: string;
    }>;
    /**
    ...
    cacheUserId(userId: string, item?: {
        conversationId?: string;
        parentMessageId?: string;
    }, opts?: SendMessageOptions): void;
    queueSendMessage(message: QueueReply, opts?: SendMessageOptions, uid?: string): Promise<string>;
    newSignal(id: string): ConversationSignal;
    sendMessage(message: string, opts?: SendMessageOptions): Promise<ChatResponse>;
...
declare function browserDispatchEvent(id: string): Promise<void>;
/**
 ...
 */
declare function browserPostEventStream(url: string, accessToken: string, body: ConversationJSONBody, timeoutMs?: number, enabledParser?: boolean): Promise<ChatError | ChatResponse>;
...
export { AChatGPTAPI, AvailableModerationModels, ChatError, ChatGPTAPI, ChatGPTAPIBrowser, ChatGPTError, ChatResponse, ContentType, ConversationJSONBody, ConversationResponseEvent, ConversationSignal, Message, MessageActionType, MessageContent, MessageFeedbackJSONBody, MessageFeedbackRating, MessageFeedbackResult, MessageFeedbackTags, MessageMetadata, Model, ModelsResult, ModerationsJSONBody, ModerationsJSONResult, OpenAIAuth, Prompt, PromptContent, QueueReply, Role, SendConversationMessageOptions, SendMessageOptions, SessionResult, User, browserDispatchEvent, browserPostEventStream, defaultChromeExecutablePath, getBrowser, getOpenAIAuth, getPage, initializeNopechaExtension, isRelevantRequest, markdownToText, maximizePage, minimizePage };
